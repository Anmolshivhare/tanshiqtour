...
class BlogCategoryDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function ($row) use ($user) {
                $editRoute   = $user->can('blog-category-edit')   ? route('admin.blog-categories.edit',    encrypt($row->id)) : '';
                $deleteRoute = $user->can('blog-category-delete') ? route('admin.blog-categories.destroy', encrypt($row->id)) : '';
                $viewRoute   = '';
    ...
    {
        $userCreate = $this->user->can('blog-category-create');
        $dataTable  = $this->builder()
            ->setTableId('blog-categories')
