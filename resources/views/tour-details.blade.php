...
@php
    ...
    // Amenities from description (fallback if no dedicated field)
    $amenities = [
        ['label' => 'Cruise Dinner & Music Event', 'available' => true],
        ['label' => 'Pick and Drop Services', 'available' => true],
        ['label' => 'Additional Services', 'available' => true],
        ['label' => 'Specialized bilingual guide', 'available' => true],
        ['label' => 'Specialized bilingual guide', 'available' => false],
        ['label' => 'Additional Services', 'available' => false],
        ['label' => 'Pick and Drop Services', 'available' => false],
        ['label' => 'Food and Drinks', 'available' => false],
    ];

    $highlights = [
        'Duis ultrices sapien a volutpat varius.',
            ...
            <div class="col-lg-8">
                ...
                <div class="td-section mb-4">
                    ...
                    <div class="td-section__text">
                        @if ($tour->description)
                            {!! $tour->description !!}
                        @else
                            <p>Experience the journey of a lifetime on this carefully curated tour package. Discover breathtaking landscapes, immerse in rich local culture, and create memories that will last forever. Our expert guides ensure every moment of your trip is exceptional.</p>
                        @endif
                    </div>
                </div>
                        ...
                        @foreach (array_chunk($amenities, 4) as $col)
                            <div class="col-md-6">
                                @foreach ($col as $amenity)
                                    <div class="td-amenity-item {{ $amenity['available'] ? 'td-amenity-item--yes' : 'td-amenity-item--no' }}">
                                    ...
                                    >
                                        <div class="accordion-body td-accordion__body">
                                            ...
                                            @if ($day->accommodation)
                                                <p><strong><i class="fas fa-bed me-1 text-primary"></i>Accommodation:</strong> {{ $day->accommodation }}</p>
                                            @endif
                                            @if ($day->meals_included)
                                                <p><strong><i class="fas fa-utensils me-1 text-success"></i>Meals:</strong> {{ $day->meals_included }}</p>
                                            @endif
                                            @if ($day->activities)
                                                <p><strong><i class="fas fa-hiking me-1 text-warning"></i>Activities:</strong> {{ $day->activities }}</p>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                ...
                @else
                    ...
                    <div class="td-section mb-4">
                        ...
                        <div class="accordion td-accordion" id="itineraryAccordion">
                            @foreach ([
                                ['day' => 1, 'title' => 'Arrival & City Orientation', 'desc' => 'Arrive at the destination and check in to your hotel. Evening orientation walk around the city highlights.'],
                                ['day' => 2, 'title' => 'Guided Heritage Tour', 'desc' => 'Full day guided tour to heritage sites, museums and local markets. Enjoy authentic local cuisine for lunch.'],
                                ['day' => 3, 'title' => 'Adventure Activities Day', 'desc' => 'Choose from a range of thrilling adventure activities. Evening bonfire and cultural performance.'],
                                ['day' => 4, 'title' => 'Leisure & Shopping', 'desc' => 'Free morning for leisure. Afternoon shopping at local bazaars. Farewell dinner at a premium restaurant.'],
                            ] as $item)
                                <div class="accordion-item td-accordion__item">
                                    ...
                                    <div id="demoDay{{ $item['day'] }}Body" class="accordion-collapse collapse {{ $item['day'] === 1 ? 'show' : '' }}">
                                        <div class="accordion-body td-accordion__body">
                                            <p>{{ $item['desc'] }}</p>
                                        </div>
                                    </div>
                ...
                @if ($tour->reviews->count() > 0)
                    <div class="td-section mb-4">
                        <h2 class="td-section__heading">Traveler Reviews</h2>
                        <div class="row g-3">
                            @foreach ($tour->reviews->take(4) as $review)
                                <div class="col-md-6">
                                    <div class="td-review-card">
                                        <div class="d-flex align-items-center gap-2 mb-2">
                                            ...
                                            <div>
                                                <div class="td-review-card__name">{{ $review->reviewer_name ?? 'Anonymous' }}</div>
                                                <div class="td-review-card__stars">
                                                    @for ($i = 1; $i <= 5; $i++)
                                                        <i class="fas fa-star {{ $i <= $review->rating ? 'text-warning' : 'text-muted' }}" style="font-size:11px;"></i>
                                                    @endfor
                                                </div>
                                            </div>
                                        </div>
                    ...
                    <div class="td-booking-card__header">
                        ...
                        @if ($tour->price_per_person)
                            <div class="td-booking-card__price">
                                ...
                                <span class="td-booking-card__price-value">₹{{ number_format($tour->price_per_person) }}</span>
                            </div>
                        @endif
                    </div>
                        ...
                        <form action="{{ route('front.contact') }}" method="GET" id="bookingForm">
                            <div class="td-booking-field mb-3">
                                ...
                                <div class="input-group">
                                    <input type="date" class="form-control td-booking-field__input" name="from_date" id="fromDate">
                            ...
                            <div class="td-booking-field mb-3">
                                ...
                                <div class="input-group">
                                    <input type="time" class="form-control td-booking-field__input" name="time" id="bookingTime">
                                    <span class="input-group-text bg-white border-start-0">
                            ...
                            <div class="td-booking-field mb-3">
                                ...
                                <select class="form-select td-booking-field__input" name="ticket_type" id="ticketType">
                            ...
                            <div class="td-booking-extras mb-3">
                                ...
                                <div class="form-check mb-1">
                                    <input class="form-check-input" type="checkbox" id="extraPerBooking" name="extras[]" value="per_booking">
                                    <label class="form-check-label td-booking-extras__check-label" for="extraPerBooking">
                                        Services Per Booking
                                    </label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" id="extraPerPerson" name="extras[]" value="per_person">
                                    <label class="form-check-label td-booking-extras__check-label" for="extraPerPerson">
                                        Services Per Person
                                    </label>
                                </div>
                            </div>
                            ...
                            <div class="td-booking-pricing mb-3">
                                <div class="td-booking-pricing__row">
                                    <span>Adult</span>
                                    <span class="td-booking-pricing__price">₹{{ number_format($tour->price_per_person ?? 1500) }}</span>
                                </div>
                                <div class="td-booking-pricing__row">
                                    <span>Youth</span>
                                    <span class="td-booking-pricing__price">₹{{ number_format(($tour->price_per_person ?? 1500) * 0.8) }}</span>
                                </div>
                                <div class="td-booking-pricing__row td-booking-pricing__row--total">
                                    <span><strong>Total</strong></span>
                                    <span class="td-booking-pricing__total">₹{{ number_format(($tour->price_per_person ?? 1500) * 1.8) }}</span>
                                </div>
                            </div>

                            <input type="hidden" name="tour" value="{{ $tour->title }}">
                            <a href="{{ route('front.contact') }}?tour={{ urlencode($tour->title) }}" class="btn td-booking-btn w-100" id="bookNowBtn">
                                Book Now &raquo;
                            </a>
                        </form>
                    </div>
